...
在config.config 下还有一个参数，为ADB_HOST 夜神模拟器第一个模拟器的HOST 是 127.0.0.1:62001
...
-  ADB_ROOT 
        这个路径为你 安卓模拟器adb工具的路径 （一般安卓电脑模拟器都有，下述为所以设置成模拟器路径即可）
```python
...
```
如果是虚拟环境，本人的是anaconda下配置的虚拟环境，路径为：
```
$ /Users/windfall/anaconda/envs/arkscript/bin/pip install -r requirement.txt
```
